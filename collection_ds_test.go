...
func TestListCrud(t *testing.T) {
	...
	size, err := list.Size()
	if err != nil {
		t.Fatalf("Failed to get size of list %v", err)
	}

	if size != 4 {
		t.Fatalf("Expected list size to be 4 but was %d", size)
	}

	index, err := list.IndexOf("test2")
	if err != nil {
		t.Fatalf("Failed to get index from list %v", err)
	}

	if index != 2 {
		t.Fatalf("Expected list index to be 2 but was %d", size)
	}

	var index2 string
	err = list.At(2, &index2)
	if err != nil {
		t.Fatalf("Failed to get content from list %v", err)
	}

	if index2 != "test2" {
		t.Fatalf("Expected list item to be test2 but was %d", size)
	}

	iter, err := list.Iterator()
	if err != nil {
		t.Fatalf("Failed to get iterator for list %v", err)
	}
	...
	for i, item := range items {
		if expected[i] != item {
			t.Fatalf("Expected item at %d to be %s but was %s", i, expected[i], item)
		}
	}
	...
	if size != 3 {
		t.Fatalf("Expected size after remove to be 3 but was %d", size)
	}
...
func TestSetCrud(t *testing.T) {
	...
	err = set.Add("test4")
	if err == nil {
		t.Fatalf("Expected adding an item twice to fail")
	}

	size, err := set.Size()
	if err != nil {
		t.Fatalf("Failed to get size of set %v", err)
	}

	if size != 4 {
		t.Fatalf("Expected set size to be 4 but was %d", size)
	}

	contains, err := set.Contains("test2")
	if err != nil {
		t.Fatalf("Failed to get contains from set %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to get iterator for set %v", err)
	}
...
func TestQueueCrud(t *testing.T) {
	...
	size, err := queue.Size()
	if err != nil {
		t.Fatalf("Failed to get size of queue %v", err)
	}

	if size != 4 {
		t.Fatalf("Expected queue size to be 4 but was %d", size)
	}

	iter, err := queue.Iterator()
	if err != nil {
		t.Fatalf("Failed to get iterator for queue %v", err)
	}
	...
	if removed != "test1" {
		t.Fatalf("Expected test1 to be pop'd but was %s", removed)
	}
...
func TestMapCrud(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Failed to Add to cMap %v", err)
	}
	...
	if size != 4 {
		t.Fatalf("Expected cMap size to be 4 but was %d", size)
	}

	iter, err := cMap.Iterator()
	if err != nil {
		t.Fatalf("Failed to get iterator for cMap %v", err)
	}
	...
	for key, item := range iter {
		...
		if !ok {
			t.Fatalf("Expected map to contain %s but didn't", key)
		}
	...
	if err != nil {
		t.Fatalf("Failed to remove from cMap %v", err)
	}
